...
public class StringSearching {
    ...
    public static List<Integer> kmp(CharSequence pattern, CharSequence text) {
        ...
        //check if pattern length is more than length
        ...
        while (i <= (text.length() - pattern.length())) {
            ...
            if (j == 0) {
                ...
                int nextIndex = failureTable[j - 1];
                i = (i + j - nextIndex);
                j = nextIndex;
            }
        }
    ...
    public static List<Integer> boyerMoore(CharSequence pattern,
            CharSequence text) {
        if (pattern == null || pattern.length() == 0) {
            throw new IllegalArgumentException("pattern is null or len 0");
        }
        if (text == null) {
            throw new IllegalArgumentException("text == null");
        }
        List<Integer> matches = new ArrayList<>();
        // check if pattern is larger than text
        if (pattern.length() > text.length()) {
            return matches;
        }
        ...
        while (i <= (text.length() - pattern.length())) {
            ...
            char currentCharacter = text.charAt(i + j);
            while ((j >= 0) && (currentCharacter == pattern.charAt(j))) {
                j--;
                if (j >= 0) {
                    currentCharacter = text.charAt(i + j);
                }
            }
            if (j == -1) {
                matches.add(i);
    ...
    public static List<Integer> rabinKarp(CharSequence pattern,
        ...
        while (i <= text.length() - pattern.length()) {
            ...
            if (i <= text.length() - pattern.length()) {
                char newC = text.charAt(i + pattern.length() - 1);
                char oldC = text.charAt(i - 1);
                textHash = updateHash(textHash, pattern.length(), oldC, newC);
            }
        }
        return matches;
    }
    ...
    public static int generateHash(CharSequence current, int length) {
        ...
        if (length < 0 || length > current.length()) {
            throw new IllegalArgumentException("len < 0 or < current length");
        }
        int hash = 0;
